...
public class CouchbaseCmisServiceFactory extends AbstractServiceFactory {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(CouchbaseCmisServiceFactory.class);
    ...
    @Override
    public void destroy() {
    	System.out.println("<CouchbaseCmisServiceFactory> destroy called by thread "+Thread.currentThread().getName()+" - class:"+Thread.currentThread().getClass()+" - id:"+Thread.currentThread().getId()+" - state:"+Thread.currentThread().getState());
    	
    	if(couchbaseService!=null) couchbaseService.close();
    ...
    private void forceRxJavaSchedulerShutdown() {
    	LOG.warn("FORCE RxJava Scheduler shutdown : Schedulers.computation="+Schedulers.computation());
    ...
    public CmisService getService(CallContext context) {
        ...
        if (service == null) {
            ...
            CouchbaseCmisService couchbaseService = new CouchbaseCmisService(repositoryManager);

            service = (CallContextAwareCmisService) wrapperManager.wrap(couchbaseService);
        ...
        for (String key : keys) {
            if (key.startsWith(PREFIX_LOGIN)) {
                ...
                if (key.endsWith(SUFFIX_READWRITE)) {
                    // read-write users
                    CouchbaseRepository fsr = repositoryManager.getRepository(repositoryId);
                    ...
                    CouchbaseRepository fsr = new CouchbaseRepository(repositoryId, root, typeManager);
